...
public class AutoShootCmd extends SequentialCommandGroup {
  ...
  public AutoShootCmd(Shooter shooter, double desiredShooterVelocity, double desiredShooterAngle) {
    ...
    // addCommands(Commands.defer(() -> getCommand(), Set.of(shooter)),
    // new InstantCommand(() -> shooter.setIndexMotorSpeed(-1)).withTimeout(1),
    // new InstantCommand(() -> shooter.setIndexMotorSpeed(0)).withTimeout(0.1),
    // new InstantCommand(() -> shooter.stopALLShooterMotors()).withTimeout(0.1));
